...
async function startServer() {
    try {
        ...
        const hasBuildFiles = fs.existsSync(path.join(publicPath, "index.html"));
        ...
        if (isProductionEnv && hasBuildFiles) {
            ...
            if (isProductionEnv && !hasBuildFiles) {
                console.warn("NODE_ENV is set to production/istehsal, but build files (dist/public) were not found. Falling back to Vite development mode.");
            }
        ...
        app.listen(PORT, "0.0.0.0", () => {
            console.log(`✅ Server is officially listening on http://0.0.0.0:${PORT}`);
        });
        ...
        // Emergency listen to satisfy platform
        app.listen(PORT, "0.0.0.0", () => {
            console.log(`Emergency listen on port ${PORT} after startup failure`);
        });
    }
}
startServer();
